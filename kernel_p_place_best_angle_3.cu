/** 
  * File:    kernel_p_rotate_molecola_3.cu 
  * 
  * Author:  Lorenzo Casalini
  * Date:     Summer 2020
  * Summary of File: 
  * This file contains a parallel implementation of function Matc Probe Shape. The parallelization here
  * is implemented at two levels. In the main function there is a OMP parallel section where each OMP thread
  * creates in the unified memory a molecule, and for each different molecule it creates a different cuda Stream
  * each stream execute the function match probe shape concurrently to all the others. Then each cuda thread runs
  * the kernel place_best_angle_p as in the file kernel_p_place_best_angle.cu   
  */
...
/**
 * An helper function which initializes an array of expansion in the unified memory of each Stream and then calls place_best_angle_p
 ...
 * @param max_range of rotations
 */
void place_in_best_angle(molecola* mol, int bond, int atom, int step, int min_range, int max_range, cudaStream_t stream){
	double* expansions;
	double best_expansion = 0;
	int best_angle;
	checkCuda(cudaMallocManaged(&expansions, 360*sizeof(double)));
	checkCuda(cudaStreamAttachMemAsync(stream, expansions, 360*sizeof(double)));
	place_in_best_angle_p<<<12,32,0,stream>>>(mol, bond, atom, step, min_range, max_range, expansions);
...
double match_probe_shape(molecola* molecola, cudaStream_t stream) {
	int* rotamer_list;
	int rotamer_index = 0;
	int n_rotamers = 0;
	int best_tile;
	...
	for (int j = 0; j < repetitions; j++) {
		for (int i = 0; i < n_rotamers; i++) {
			...
			if (fragment_size(molecola, rotamer_index, 1) < threshold) {
				place_in_best_angle(molecola, rotamer_index, molecola->bonds[2 * rotamer_index], low_precision_step, 0, 360,stream);
			} 
			else {
				if (enable_refiniment) {
					best_tile = find_best_tile(molecola, n_tiles, rotamer_index, molecola->bonds[2 * rotamer_index]);
					place_in_best_angle(molecola, rotamer_index, molecola->bonds[2 * rotamer_index], high_precision_step, best_tile*tile_size, (best_tile + 1)*tile_size,stream);
				}
				else {
					place_in_best_angle(molecola, rotamer_index, molecola->bonds[2 * rotamer_index], high_precision_step, 0, 360,stream);
				}
			}
			if (fragment_size(molecola, rotamer_index, 2) < threshold) {
				place_in_best_angle(molecola, rotamer_index, molecola->bonds[2 * rotamer_index + 1], low_precision_step, 0, 360,stream);
			}
			else {
				if (enable_refiniment) {
					best_tile = find_best_tile(molecola, n_tiles, rotamer_index, molecola->bonds[2 * rotamer_index + 1]);
					
					place_in_best_angle(molecola, rotamer_index, molecola->bonds[2 * rotamer_index + 1], high_precision_step, best_tile*tile_size, (best_tile + 1)*tile_size,stream);
				}
				else {
					place_in_best_angle(molecola, rotamer_index, molecola->bonds[2 * rotamer_index + 1], high_precision_step, 0, 360,stream);
				}
			}
		}
	}
