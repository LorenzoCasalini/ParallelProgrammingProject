...
__global__ void rotate_molecola_kernel(molecola* molecola, queue* q1, int rotamer, int atom, int angle){
	/**
	 * In this kernel each thread rotated an atom which index is in queue
	 * @param molecola
...
void rotate_molecola(molecola* molecola, int rotamer, int atom, int angle) {
	/**
	 * This function mallocs a queue in the global memory, then populates it calling find_adjacent_node and then
	 * calls the kernel rotate_molecola_kernel
	 * @param molecola
	 * @param rotamer that will be the axis of rotation
	 * @param atom is needed to understand which fragment to rotate
	 * @param angle
	 */
	queue* q1;
	checkCuda(cudaMallocManaged(&q1, sizeof(queue)));
	find_adjacent_nodes(molecola, q1, atom, rotamer);
	rotate_molecola_kernel<<<1,1024>>>(molecola, q1,rotamer, atom, angle);
	checkCuda(cudaDeviceSynchronize());
	checkCuda(cudaFree(q1));
}
...
void place_in_best_angle(molecola* mol, int bond, int atom, int step, int min_range, int max_range) {
	/**
	 * Places the molecola in the best angle
	 * @param mol 
...
int find_best_tile(molecola* mol, int n_tiles, int bond, int atom) {
	/**
	 * Places molecola in the middle of each tile, then evaluate the expansion of the molecola with the fragment placed in the tile, at the end returns the index of the tile with the best expansion
	 * @param mol
