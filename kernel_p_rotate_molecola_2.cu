/** 
  ...
  * Date:     Summer 2020
  * Summary of File: 
  * This file contains a parallel implementation of function Match Probe Shape. The parallel part is
  * the rotation of the molecule, in which every atom of a fragment is rotated in parallel. The difference between this file and 
  * "kernel_p_rotate_molecola.cu" is the fact that in the function "rotate_molecola_kernel" the variable queue is shared, so
  * there is no need to call a cudaMalloc every time rotate_molecola is called. 
  */ 
 ...
 */
__device__ __host__ void bfs(int bond_index,molecola*molecola,queue* queue){
 ...
 * @param bond_index 
 */
__device__ __host__ void find_adjacent_nodes(molecola* molecola, queue* queue, int atom, int bond_index) {
...
__device__ void normalise(double* x, double* y, double* z) {
 ...
 * @param angle 
 */
__device__ void rotate_atom(molecola*molecola, int atom, int rotamer, int angle) {
 ...
 * @return distance 
 */
double distance(molecola* molecola, int index_1, int index_2) {
	double distance;
...
/**
 * In this kernel we declare a shared queue. This queue is common to all threads
 * belonging to the same block, so only one thread populates it, then each atoms is 
 * rotated by a different thread. 
 * @param molecola
 * @param q1 queue containing the atoms to rotate
 * @param rotamer that will be the axis of rotation
 * @param atom is needed to understand which fragment to rotate
 * @param angle
 */
__global__ void rotate_molecola(molecola* molecola, int rotamer, int atom, int angle){
	...
	__syncthreads();
	if(thread_id< q1.tail){
		rotate_atom(molecola, q1.elements[thread_id],rotamer,angle);
	}
}
 ...
 * @param molecola
 */
bool is_ligand_feasible(molecola* molecola) {
	for (int i = 0; i < molecola->n_atoms; i++) {
		for (int j = 0; j < molecola->n_atoms; j++) {
			if (j > i) {
...
/**
 * Places the molecola in the best angle. For each angle it calls the kernel rotate_molecola, than evaluates the	
 * expansion of the molecule and if it is bigger than the best one seen so far, it saves the angle and the 
 * expansion. At the end it rotates  the molecule in the best angle found. 
 * @param mol 	
...
void place_in_best_angle(molecola* mol, int bond, int atom, int step, int min_range, int max_range) {
	...
	for (int i = min_range; i <= max_range; i += step) {
		rotate_molecola<<<1,1024>>>(mol, bond, atom, step);
		checkCuda(cudaDeviceSynchronize());
		if (is_ligand_feasible(mol)) {
	...
	rotate_molecola<<<1,1024>>>(mol, bond, atom, (360 - max_range) + best_expansion_angle);
	checkCuda(cudaDeviceSynchronize());
}
...
int find_best_tile(molecola* mol, int n_tiles, int bond, int atom) {
	...
	rotate_molecola<<<1,1024>>>(&mol2, bond, atom, tile_size / 2);
	checkCuda(cudaDeviceSynchronize());
	...
	for (int i = 1; i < n_tiles; i++) {
		index = tile_size / 2;
		rotate_molecola<<<1,1024>>>(&mol2, bond, atom, tile_size);
		checkCuda(cudaDeviceSynchronize());
		if (is_ligand_feasible(&mol2)) {
