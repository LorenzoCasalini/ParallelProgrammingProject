/** 
  ...
  * the measurement of the expansion of the molecule after its rotations.
  */ 
...
/**
 * Given a molecola creates an array in shared memory of distances, each thread
 * calculates the sum of the distances from the corrisponding atom to all the other
 * atoms and then puts them in the array. Then one thread sums all the distances.
 * @param molecola 
 * @param expansion a pointer where to put the sum of all the distances  
 */
__global__ void measure_expansion(molecola* molecola, double* expansion){
	__shared__ double distances[200];
	__shared__ double expansion_tmp;
	int thread_id;
	thread_id = threadIdx.x + blockIdx.x*blockDim.x;
	if(thread_id < molecola->n_atoms){
		distances[thread_id] = 0;
		for(int i = 0; i < molecola->n_atoms; i++){
			distances[thread_id] += distance(molecola, thread_id, i);
		}
	}
	__syncthreads();
	if(thread_id == 0){
		expansion_tmp = 0;
		for(int i = 0; i < molecola->n_atoms; i++){
			expansion_tmp = expansion_tmp + distances[i];
		}
		*expansion = expansion_tmp;
	}
}
...
int find_best_tile(molecola* mol, int n_tiles, int bond, int atom) {
	...
	if (is_ligand_feasible(&mol2)) {
		measure_expansion<<<1,1>>>(&mol2, expansion);
		checkCuda(cudaDeviceSynchronize());
	...
	for (int i = 1; i < n_tiles; i++) {
		...
		if (is_ligand_feasible(&mol2)) {
			measure_expansion<<<1,256>>>(&mol2,expansion);
			checkCuda(cudaDeviceSynchronize());
			if (*expansion > best_expansion) {
				best_expansion = *expansion;
...
double match_probe_shape(molecola* molecola) {
	...
	double espansione;
	double* expansion_2;
	checkCuda(cudaMallocManaged(&expansion_2,sizeof(double)));
	...
	free(rotamer_list);
	measure_expansion<<<1,200>>>(molecola,expansion_2);
	checkCuda(cudaDeviceSynchronize());
	espansione = *expansion_2;
	checkCuda(cudaFree(expansion_2));
	return espansione;
}
...
int main() {
	...
	double espansione;
	double* espansion;
	checkCuda(cudaMallocManaged(&espansion,sizeof(double)));
	molecola* m1;
	...
	for (int i = 0; i < n_molecole; i++) {
		*espansion = 0;
		...
		measure_expansion<<<1,200>>>(m1,espansion);
		checkCuda(cudaDeviceSynchronize());
		printf("Before expansion Molecola: %s , espansion: %f\n", m1->name,*espansion);
		espansione = match_probe_shape(m1);
