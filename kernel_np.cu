...
/** 
  ...
  * Date:     Summer 2020
  * Summary of File: 
  * This file contains a sequential implementation of function Match Probe Shape.
  * 
  */ 
...
void bfs(int bond_index, molecola*molecola, queue* queue){
	...
	while(queue->head < queue->tail){
		for(int i = 0; i<n_bonds;i++){
			//Check if a bound contains the node we are expanding now, if the bound exists we add the other node of the bound in the queue unless it is already present in the queue  
		    if(i!=bond_index){
...
int* find_rotamers(molecola* molecola, int* number_of_rotamers) {
	...
	int* rotamer_list;
	int rotamer_index = 0;
	x = (bool*)malloc(size* sizeof(bool));
...
int find_best_tile(molecola* mol, int n_tiles, int bond, int atom) {
	molecola mol2;
	//create a copy of mol in the stack, is useful because we can rotate it without modifying the original one
	mol2 = *mol;
	int tile_size;
	double expansion;
	double best_expansion=0;
	...
	// The first rotation is of (tile_size/2) angle because i want to place the fragment in the middle of the tile 
...
int main() {
	...
	for (int i = 0; i < n_molecole; i++) {
		m1 = (molecola*) malloc(sizeof(molecola));
